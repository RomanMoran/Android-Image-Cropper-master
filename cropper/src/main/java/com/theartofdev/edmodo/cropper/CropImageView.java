...
public class CropImageView extends FrameLayout {
    ...
    /**
     * The Url that the image was loaded from (if loaded from Internet)
     */
     ...
     * Task used to load bitmap async from UI thread
     */
    private WeakReference<GifUrlLoadingWorkerTask> mGifUrlLoadingWorkerTask;

    /**
     * Task used to crop bitmap async from UI thread
     */
    private WeakReference<BitmapCroppingWorkerTask> mBitmapCroppingWorkerTask;
    private WeakReference<MultipleBitmapCroppingWorkerTask> mMultipleBitmapCroppingWorkerTask;
    // endregion
    ...
    public void setImageUrlFromGlide(String url) {
        if (url != null) {
            GifUrlLoadingWorkerTask currentTask =
                    mGifUrlLoadingWorkerTask != null ? mGifUrlLoadingWorkerTask.get() : null;
            if (currentTask != null) {
                // cancel previous loading (no check if the same URI because camera URI can be the same for
                // different images)
                currentTask.cancel(true);
            ...
            mRestoreDegreesRotated = 0;
            mCropOverlayView.setInitialCropWindowRect(null);
            mGifUrlLoadingWorkerTask = new WeakReference<>(new GifUrlLoadingWorkerTask(this, url));
            mGifUrlLoadingWorkerTask.get().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
            setProgressBarVisibility();
        }
    }
    ...
    void onSetImageGifUriAsyncComplete(GifUrlLoadingWorkerTask.ResultGif result) {
        ...
        if (result.error == null) {
            mInitialDegreesRotated = result.degreesRotated;
            setGlideGifBitmap(result.url, result.loadSampleSize, result.degreesRotated);
        }

        OnSetImageUriCompleteListener listener = mOnSetImageUriCompleteListener;
        if (listener != null) {
            listener.onSetImageUriComplete(this, null, result.error);
        }
    }
    ...
    void onMultipleImageCroppingAsyncComplete(MultipleBitmapCroppingWorkerTask.Result result) {
    ...
    public void setGlideGifBitmap(final String imageUrl, final int loadSampleSize, final int degreesRotated) {
        if (mBitmap == null) {
    ...
    public void startCropWorkerTaskForManyUris(
            int reqWidth,
            int reqHeight,
            RequestSizeOptions options,
        ...
        if (bitmap != null) {
            ...
            if (mLoadedImageUri != null
                    && (mLoadedSampleSize > 1 || options == RequestSizeOptions.SAMPLING)) {
                mMultipleBitmapCroppingWorkerTask =
                        new WeakReference<>(
                                new MultipleBitmapCroppingWorkerTask(
                                        this,
                                        mLoadedImageUri,
                                        getCropPoints(),
                                        mDegreesRotated,
            ...
            mMultipleBitmapCroppingWorkerTask.get().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
            setProgressBarVisibility();
        }
    }
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (mGifUrlLoadingWorkerTask != null) {
            GifUrlLoadingWorkerTask task = mGifUrlLoadingWorkerTask.get();
            if (task != null) {
                bundle.putParcelable("LOADING_IMAGE_URI", task.getUri());
            }
        }
    ...
    public interface OnCropImageCompleteListener {
        ...
        void onMultipleCropImageComplete(CropImageView view, List<Bitmap> result);
    }
    // endregion
