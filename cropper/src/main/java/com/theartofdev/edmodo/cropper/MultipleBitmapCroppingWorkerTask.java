 ...
 */
final class MultipleBitmapCroppingWorkerTask
        extends AsyncTask<Void, Void, MultipleBitmapCroppingWorkerTask.Result> {
    ...
    MultipleBitmapCroppingWorkerTask(
     ...
     */
    @Override
    protected MultipleBitmapCroppingWorkerTask.Result doInBackground(Void... params) {
        try {
            if (!isCancelled()) {
                ...
                for (int i = 0; i < mSavedBitmaps.size(); i++) {
                    ...
                    if (mUri != null) {
                        ...
                        return new Result(new ArrayList<Bitmap>(), 1);
                    }*/
                    ...
                    if (mBitmap != null && bitmap != null) {
                        ...
                        BitmapUtils.writeBitmapToUri(
                                mContext, bitmap, mUri, mSaveCompressFormat, mSaveCompressQuality);
                        if (bitmap != null) {
                            bitmap.recycle();
                        }
                        uris.add(mUri);
                ...
                return new Result(bitmaps, uris, bitmapSampled.sampleSize);
        ...
        if (result != null) {
            ...
            if (!isCancelled()) {
                ...
                if (cropImageView != null) {
                    completeCalled = true;
                    cropImageView.onMultipleImageCroppingAsyncComplete(result);
                }
            }
            if (!completeCalled && result.bitmaps != null) {
                // fast release of unused bitmap
                for (int i = 0; i < result.bitmaps.size(); i++) {
                    Bitmap bitmap = result.bitmaps.get(i);
                    bitmap.recycle();
                }
            }
        }
    ...
    static final class Result {
        ...
        Result(List<Uri> uris, int sampleSize) {
            this.bitmaps = null;
            this.uris = uris;
            this.error = null;
            this.isSave = false;
        ...
        Result(ArrayList<Bitmap> bitmaps, int sampleSize) {
        ...
        Result(ArrayList<Bitmap> bitmaps, ArrayList<Uri> uris, int sampleSize) {
            this.bitmaps = bitmaps;
            this.uris = uris;
            this.error = null;
            this.isSave = false;
